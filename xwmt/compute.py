...
def calc_hldotmass(xgrid,dd):
    ...
    if dd['boundary']['mass']: # If boundary flux specified as mass rather than tracer flux
        scalar_i = xgrid.interp(dd['scalar']['array'],'Z',boundary='extrapolate').chunk({'lev_outer':-1})
        Jlmass = Jlmass_from_Qm_lm_l(hldotmass,dd['boundary']['scalar_in_mass'],scalar_i)
...
def calc_hldot_tendency(xgrid, dd):
    ...
    if dd['tendency']['extensive']:
        ...
        hldot = hldot_from_ldot_h(dd['tendency']['array'], xgrid.get_metric(dd['tendency']['array'],'Z'))
...
def get_density(ds, grid, density_str='sigma0'):
    ...
    sa = xr.apply_ufunc(gsw.SA_from_SP, ds['so'], p, grid['lon'], grid['lat'], dask='parallelized')
...
def calc_F_transformed(F, l, xgrid, bins):
    ...
    F_transformed = xgrid.transform(F, 'Z', target=bins, target_data=l,
...
def calc_G(F, l, grid, bins, method='xhistogram'):
    ...
    if method == 'xhistogram':
        ...
        G = histogram(l.where(~np.isnan(F)), bins = [bins], dim = ['x','y','lev'], 
                      weights = (F*grid['areacello']).where(~np.isnan(F)))/np.diff(bins)

    elif method == 'xgcm':

        G = (calc_F_transformed(F,l.copy().rename(l.name+'_bin'),xgrid,bins)*grid['areacello']).sum(['x','y'])
...
def lbin_define(lmin,lmax,delta_l):
    ...
    return np.arange(lmin-delta_l/2.,lmax+delta_l/2.,delta_l)
