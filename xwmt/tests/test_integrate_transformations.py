...
def test_functional_3d_theta_default():
    total_wmt = wmt.integrate_transformations("heat", **kwargs)['process_material_derivative']
    assert np.isclose(
...
def test_functional_3d_salt_default():
    total_wmt = wmt.integrate_transformations("salt", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        78005966.15053421,
    )
...
def test_functional_3d_sigma0_default():
    total_wmt = wmt.integrate_transformations("sigma0", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        550052085.4891472,
    )
...
def test_functional_3d_sigma1_default():
    total_wmt = wmt.integrate_transformations("sigma1", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        550052085.4891474,
    )
...
def test_functional_3d_sigma2_default():
    total_wmt = wmt.integrate_transformations("sigma2", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        550065820.7692934,
    )
...
def test_functional_3d_sigma3_default():
    total_wmt = wmt.integrate_transformations("sigma3", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        550052085.4891478,
    )
...
def test_functional_3d_sigma4_default():
    total_wmt = wmt.integrate_transformations("sigma4", **kwargs)['process_material_derivative']
    assert np.isclose(
        total_wmt.sum().values,
        550052085.489147,
    )
